...
const char * const BITCOIN_CONF_FILENAME = "winstarrxcoin.conf";
...
#else
    const char* pszModule = "winstarrxcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "WinstarRxcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/WinstarRxcoin";
#else
    // Unix
    return pathRet / ".winstarrxcoin";
#endif
#endif
}
