WinstarRxcoin Core version *0.15.1* is now available from:

  <https://download.winstarrxcoin.org/winstarrxcoin-0.15.1/>
...
A number of changes to the way WinstarRxcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 winstars per kB (0.00001 WRX). This is to prepare for dropping the minimum transaction fee to 0.1 winstars per kB (0.0001 WRX) in the next release.
